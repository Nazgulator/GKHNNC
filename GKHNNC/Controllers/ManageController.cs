    ...
    public class ManageController : Controller
    {
        ...
        public ActionResult RolesAndUsers()
        {
           ...
           foreach (ApplicationUser U in SS)
            {
                IList<string> rol = UserManager.GetRoles(U.Id);
                if (rol.Count != 0)
                {
                    Roles.Add(rol[0]);
                }
        ...
        public ActionResult ToAdmin (string id, string roll)
        {
            
            if (roll == "Пользователь")
            {
                UserManager.RemoveFromRole(id, "Пользователь");
                UserManager.AddToRole(id, "Администратор");
            }
            else
            {
                if (roll == "Администратор")
                {
                    UserManager.RemoveFromRole(id, "Администратор");
                    UserManager.AddToRole(id, "Пользователь");
                }
            }
           ...
           // RedirectToAction("RolesAndUsers");
            return View("RolesAndUsers",SS);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public  ActionResult EditUserName([Bind(Include = "UserName")] EditUserNameViewModel editUserName)
        {
            if (!ModelState.IsValid)
            {
