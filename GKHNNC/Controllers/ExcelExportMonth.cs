...
"выполненные работы по содержанию и текущему ремонту общего имущества в многоквартирном доме, расположенном по адресу "+ Adres;
            ...
            from++;
            WS.Cells[startStroka, 1] = "2.Всего за период с 01." + month + "." + year + " по 30." + month + "." + year + "выполнено работ(оказано услуг) на общую сумму " + Summa + "рублей.";
            range = WS.get_Range("A" + from, "E" + from);
            range.Merge(Type.Missing);
            range.Font.Bold = false;
            range.Font.Size = 10;
        ...
        public static void SFORMIROVATAKT(List<CompleteWork> CompleteWorks,List<VipolnennieUslugi> VipolnennieUslugi, string Month, string GEU, string Year, string Adres, string Nachalnik, string Prikaz,string patch,string Summa)
        {
            ...
            string Res = Adres.Remove(0, a + 1);
            Res = Res.Replace(" ", "");
            if (Ad.Contains("МОЛОДЕЖИ")) { Ad = "БУЛЬВАР "+Ad; }
            if (Ad.Contains("МОРСКОЙ")) { Ad = Ad+ " ПРОСПЕКТ"; }
